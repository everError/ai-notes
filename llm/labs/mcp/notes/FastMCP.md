...
- **유연성**: 로컬 개발에 용이한 `STDIO` 방식과 웹 배포에 적합한 `Streamable HTTP` 방식을 지원합니다. 구 `SSE(Server-Sent Events)` 전송은 2026-07-28 스펙에서 폐기되었으므로 신규 구현에는 사용하지 않습니다.
...
### ## 2026-07-28 스펙 전환 시 확인할 점

프레임워크가 프로토콜 세부를 감춰 주므로 `@mcp.tool()` 로 도구를 정의하는 방식 자체는 바뀌지 않습니다. 다만 아래 항목은 프레임워크가 대신 처리해 주지 않습니다.

- **전송 선택** — 신규 원격 서버는 Streamable HTTP 를 사용합니다. SSE 전송은 폐기되었습니다.
- **서버에 남긴 상태** — 연결 단위로 값을 들고 있는 코드가 있다면 stateless 코어에서 동작하지 않습니다. 인가 토큰 클레임, 요청 파라미터, `requestState` 중 하나로 옮겨야 합니다.
- **사용자 확인 절차** — 도구 실행 도중 사용자에게 되묻는 로직은 MRTR 형태로 재작성이 필요합니다.
- **도구 목록 캐싱** — 사용자마다 노출 도구가 달라지는 서버라면 `cacheScope` 설정을 확인해야 합니다.

구체적인 절차는 [MCP_Stateless_Migration](MCP_Stateless_Migration.md) 을 참고합니다.
